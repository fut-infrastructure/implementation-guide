...
- `COMMUNICATION_JSON_PATH_IS_ONLY_ALLOWED_FOR_CATEGORY_S_CURRENT_IS_S`: This JSON patch type is only allowed for category '%s' (current is '%s')
- `COMMUNICATION_EXACTLY_ONE_MESSAGE_CATEGORY_IS_EXPECTED_ACTUAL_S`: Exactly one message category is expected. Actual: %s
...
- `PATIENT_ADDRESS_VALIDATION_FAILED`: Patient address validation failed: %s
- `MUST_HAVE_CRN_IDENTIFIER`: No CPR identifier found
