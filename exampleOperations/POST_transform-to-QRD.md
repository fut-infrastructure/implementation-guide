`POST [base]/$transform-to-QRD`

__Header__
```
Accept-Charset: utf-8
Authorization: Bearer eyJhbGciOiJub25lIn0.eyJyZWFsbV9hY2Nlc3MiOnsicm9sZXMiOlsiJHRyYW5zZm9ybS1mcm9tLVFSRCIsIiR0cmFuc2Zvcm0tdG8tUVJEIiwiJHRlc3Qtb25seS1jcmVhdGUiXX0sInVzZXJfdHlwZSI6IlNZU1RFTSJ9.
Accept: application/fhir+json;q=1.0, application/json+fhir;q=0.9
User-Agent: HAPI-FHIR/5.0.2 (FHIR Client; FHIR 3.0.2/DSTU3; apache)
Accept-Encoding: gzip
Content-Type: application/fhir+json; charset=UTF-8
```

__Body__:
```json
{
  "resourceType": "Parameters",
  "parameter": [
    {
      "name": "bundle",
      "resource": {
        "resourceType": "Bundle",
        "type": "collection",
        "entry": [
          {
            "fullUrl": "https://questionnaire.cit-document-transformation-1046.local/fhir/Questionnaire/66697",
            "resource": {
              "resourceType": "Questionnaire",
              "id": "66697",
              "meta": {
                "versionId": "1",
                "profile": [
                  "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-questionnaire"
                ]
              },
              "extension": [
                {
                  "url": "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-questionnaire-type",
                  "valueCodeableConcept": {
                    "coding": [
                      {
                        "system": "http://ehealth.sundhed.dk/cs/questionnaire-types",
                        "code": "TBD"
                      }
                    ]
                  }
                },
                {
                  "url": "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-modifier-role",
                  "extension": [
                    {
                      "url": "reference",
                      "valueReference": {
                        "reference": "https://organization.cit-document-transformation-1046.local/fhir/Organization/82367"
                      }
                    },
                    {
                      "url": "role",
                      "valueCodeableConcept": {
                        "coding": [
                          {
                            "system": "http://ehealth.sundhed.dk/cs/modifier-role",
                            "code": "owner"
                          }
                        ]
                      }
                    }
                  ]
                }
              ],
              "version": "d883011a-eb28-43a5-a51b-5c9adb82b60d",
              "name": "fa535531-b008-41dd-853d-fc2755a1877a",
              "status": "active",
              "item": [
                {
                  "linkId": "13e60bbd-7839-4d9d-8616-269b47ab610a,1.2.208.176.1.5,Sundhedsdatastyrelsen",
                  "type": "text"
                },
                {
                  "extension": [
                    {
                      "url": "http://hl7.org/fhir/StructureDefinition/minValue",
                      "valueDateTime": "2020-12-21T13:53:21+00:00"
                    },
                    {
                      "url": "http://hl7.org/fhir/StructureDefinition/maxValue",
                      "valueDateTime": "2020-12-21T13:53:21+00:00"
                    }
                  ],
                  "linkId": "512e47ba-b185-4bdd-a554-c6da89c1fff5,1.2.208.176.1.5,Sundhedsdatastyrelsen",
                  "type": "dateTime",
                  "required": true,
                  "repeats": true
                },
                {
                  "extension": [
                    {
                      "url": "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-questionnaire-sliderStepValue",
                      "valueInteger": 5
                    },
                    {
                      "url": "http://hl7.org/fhir/StructureDefinition/minValue",
                      "valueInteger": 100
                    },
                    {
                      "url": "http://hl7.org/fhir/StructureDefinition/maxValue",
                      "valueInteger": 1000
                    }
                  ],
                  "linkId": "021f2f21-3b14-430a-ac0f-c2e4078826c8,1.2.208.176.1.5,Sundhedsdatastyrelsen",
                  "type": "decimal",
                  "required": true,
                  "repeats": true
                },
                {
                  "extension": [
                    {
                      "url": "http://hl7.org/fhir/StructureDefinition/minValue",
                      "valueInteger": 100
                    },
                    {
                      "url": "http://hl7.org/fhir/StructureDefinition/maxValue",
                      "valueInteger": 1000
                    }
                  ],
                  "linkId": "a3974758-8102-4f18-b778-f6c5e92a865b,1.2.208.176.1.5,Sundhedsdatastyrelsen",
                  "type": "decimal",
                  "required": true,
                  "repeats": true
                },
                {
                  "extension": [
                    {
                      "url": "http://hl7.org/fhir/StructureDefinition/questionnaire-minOccurs",
                      "valueInteger": 1
                    },
                    {
                      "url": "http://hl7.org/fhir/StructureDefinition/questionnaire-maxOccurs",
                      "valueInteger": 2
                    }
                  ],
                  "linkId": "3c230c42-8190-45b2-b775-006ef06515a2,1.2.208.176.1.5,Sundhedsdatastyrelsen",
                  "type": "choice",
                  "required": true,
                  "repeats": true,
                  "option": [
                    {
                      "valueCoding": {
                        "system": "urn:oid:1.2.208.176.1.5",
                        "code": "Option1",
                        "display": "Sundhedsdatastyrelsen"
                      }
                    },
                    {
                      "valueCoding": {
                        "system": "urn:oid:1.2.208.176.1.5",
                        "code": "Option2",
                        "display": "Sundhedsdatastyrelsen"
                      }
                    }
                  ]
                },
                {
                  "extension": [
                    {
                      "url": "http://hl7.org/fhir/StructureDefinition/questionnaire-minOccurs",
                      "valueInteger": 1
                    },
                    {
                      "url": "http://hl7.org/fhir/StructureDefinition/questionnaire-maxOccurs",
                      "valueInteger": 1
                    }
                  ],
                  "linkId": "01128d5b-8d31-4dca-8986-93088c82af01,1.2.208.176.1.5,Sundhedsdatastyrelsen",
                  "type": "choice",
                  "required": true,
                  "repeats": true,
                  "option": [
                    {
                      "valueCoding": {
                        "system": "urn:oid:1.2.208.176.1.5",
                        "code": "Option1",
                        "display": "Sundhedsdatastyrelsen"
                      }
                    },
                    {
                      "valueCoding": {
                        "system": "urn:oid:1.2.208.176.1.5",
                        "code": "Option2",
                        "display": "Sundhedsdatastyrelsen"
                      }
                    }
                  ]
                }
              ]
            }
          },
          {
            "fullUrl": "https://measurement.cit-document-transformation-1046.local/fhir/QuestionnaireResponse/51215",
            "resource": {
              "resourceType": "QuestionnaireResponse",
              "id": "51215",
              "meta": {
                "versionId": "1",
                "profile": [
                  "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-questionnaireresponse"
                ]
              },
              "basedOn": [
                {
                  "reference": "https://careplan.cit-document-transformation-1046.local/fhir/ProcedureRequest/43759"
                }
              ],
              "questionnaire": {
                "reference": "https://questionnaire.cit-document-transformation-1046.local/fhir/Questionnaire/66697"
              },
              "status": "completed",
              "subject": {
                "reference": "https://patient.cit-document-transformation-1046.local/fhir/Patient/35761"
              },
              "context": {
                "reference": "https://careplan.cit-document-transformation-1046.local/fhir/EpisodeOfCare/88768"
              },
              "authored": "2020-12-21T13:53:21+00:00",
              "source": {
                "reference": "https://patient.cit-document-transformation-1046.local/fhir/Patient/45689"
              },
              "item": [
                {
                  "linkId": "13e60bbd-7839-4d9d-8616-269b47ab610a,1.2.208.176.1.5,Sundhedsdatastyrelsen",
                  "answer": [
                    {
                      "valueString": "Text Answer"
                    }
                  ]
                },
                {
                  "linkId": "512e47ba-b185-4bdd-a554-c6da89c1fff5,1.2.208.176.1.5,Sundhedsdatastyrelsen",
                  "answer": [
                    {
                      "valueDateTime": "2020-12-21T13:53:21+00:00"
                    }
                  ]
                },
                {
                  "linkId": "021f2f21-3b14-430a-ac0f-c2e4078826c8,1.2.208.176.1.5,Sundhedsdatastyrelsen",
                  "answer": [
                    {
                      "valueDecimal": 500
                    }
                  ]
                },
                {
                  "linkId": "a3974758-8102-4f18-b778-f6c5e92a865b,1.2.208.176.1.5,Sundhedsdatastyrelsen",
                  "answer": [
                    {
                      "valueDecimal": 260
                    }
                  ]
                },
                {
                  "linkId": "3c230c42-8190-45b2-b775-006ef06515a2,1.2.208.176.1.5,Sundhedsdatastyrelsen",
                  "answer": [
                    {
                      "valueCoding": {
                        "system": "urn:oid:1.2.208.176.1.5",
                        "code": "Option2",
                        "display": "Sundhedsdatastyrelsen"
                      }
                    }
                  ]
                },
                {
                  "linkId": "01128d5b-8d31-4dca-8986-93088c82af01,1.2.208.176.1.5,Sundhedsdatastyrelsen",
                  "answer": [
                    {
                      "valueCoding": {
                        "system": "urn:oid:1.2.208.176.1.5",
                        "code": "Option1",
                        "display": "Sundhedsdatastyrelsen"
                      }
                    }
                  ]
                }
              ]
            }
          },
          {
            "fullUrl": "https://document-transformation.cit-document-transformation-1046.local/fhir/Composition/9201",
            "resource": {
              "resourceType": "Composition",
              "id": "9201",
              "meta": {
                "versionId": "1",
                "profile": [
                  "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-composition"
                ]
              },
              "extension": [
                {
                  "url": "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-composition-authorOrganization",
                  "valueReference": {
                    "reference": "https://organization.cit-document-transformation-1046.local/fhir/Organization/61854"
                  }
                }
              ],
              "identifier": {
                "system": "urn:oid:1.2.208.184",
                "value": "IdentifierValue1"
              },
              "status": "preliminary",
              "type": {
                "coding": [
                  {
                    "system": "http://loinc.org",
                    "code": "74465-6"
                  }
                ]
              },
              "subject": {
                "reference": "https://patient.cit-document-transformation-1046.local/fhir/Patient/24541"
              },
              "date": "2020-12-21T13:53:21+00:00",
              "author": [
                {
                  "reference": "https://patient.cit-document-transformation-1046.local/fhir/Patient/24541"
                }
              ],
              "title": "CompTitle",
              "confidentiality": "N",
              "custodian": {
                "reference": "https://organization.cit-document-transformation-1046.local/fhir/Organization/58427"
              },
              "relatesTo": [
                {
                  "code": "appends",
                  "targetReference": {
                    "reference": "https://document-transformation.cit-document-transformation-1046.local/fhir/Composition/33935"
                  }
                }
              ],
              "section": [
                {
                  "entry": [
                    {
                      "reference": "https://questionnaire.cit-document-transformation-1046.local/fhir/Questionnaire/66697"
                    },
                    {
                      "reference": "https://measurement.cit-document-transformation-1046.local/fhir/QuestionnaireResponse/51215"
                    },
                    {
                      "reference": "https://document-transformation.cit-document-transformation-1046.local/fhir/DocumentReference/45777"
                    },
                    {
                      "reference": "https://document-transformation.cit-document-transformation-1046.local/fhir/Composition/33935"
                    }
                  ]
                }
              ]
            }
          },
          {
            "fullUrl": "https://document-transformation.cit-document-transformation-1046.local/fhir/Composition/33935",
            "resource": {
              "resourceType": "Composition",
              "id": "33935",
              "meta": {
                "versionId": "1",
                "profile": [
                  "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-composition"
                ]
              },
              "contained": [
                {
                  "resourceType": "Patient",
                  "id": "1",
                  "meta": {
                    "profile": [
                      "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-patient"
                    ]
                  },
                  "identifier": [
                    {
                      "system": "urn:oid:1.2.208.176.1.2",
                      "value": "0101010001"
                    }
                  ],
                  "gender": "female",
                  "address": [
                    {
                      "use": "home",
                      "line": [
                        "Scot 2"
                      ]
                    }
                  ],
                  "managingOrganization": {
                    "reference": "https://organization.cit-document-transformation-1046.local/fhir/Organization/3394"
                  }
                },
                {
                  "resourceType": "Practitioner",
                  "id": "2",
                  "meta": {
                    "profile": [
                      "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-practitioner"
                    ]
                  },
                  "name": [
                    {
                      "family": "18333117-1cfc-4a35-8346-9c9b6cd001d5"
                    }
                  ]
                }
              ],
              "status": "preliminary",
              "type": {
                "coding": [
                  {
                    "system": "http://loinc.org",
                    "code": "74468-0"
                  }
                ]
              },
              "subject": {
                "reference": "#1"
              },
              "date": "2020-12-21T13:53:21+00:00",
              "author": [
                {
                  "reference": "#2"
                }
              ],
              "title": "dd84270a-f8e4-4640-a7dd-6eaa86a17222"
            }
          },
          {
            "fullUrl": "https://document-transformation.cit-document-transformation-1046.local/fhir/DocumentReference/45777",
            "resource": {
              "resourceType": "DocumentReference",
              "id": "45777",
              "meta": {
                "versionId": "1",
                "profile": [
                  "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-documentreference"
                ]
              },
              "status": "current",
              "type": {
                "coding": [
                  {
                    "system": "http://loinc.org",
                    "code": "69730-0"
                  }
                ]
              },
              "indexed": "2020-12-21T13:53:21.305+00:00",
              "content": [
                {
                  "attachment": {
                    "url": "http://some.com/"
                  }
                },
                {
                  "attachment": {
                    "url": "htttp://foo-url.com"
                  }
                }
              ]
            }
          },
          {
            "fullUrl": "https://patient.cit-document-transformation-1046.local/fhir/Patient/24541",
            "resource": {
              "resourceType": "Patient",
              "id": "24541",
              "meta": {
                "versionId": "1",
                "profile": [
                  "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-patient"
                ]
              },
              "identifier": [
                {
                  "system": "urn:oid:1.2.208.176.1.2",
                  "value": "0101010001"
                }
              ],
              "name": [
                {
                  "family": "Family1",
                  "given": [
                    "Given1",
                    "Given1"
                  ],
                  "prefix": [
                    "Prefix1",
                    "Prefix2",
                    "Prefix3"
                  ]
                }
              ],
              "telecom": [
                {
                  "system": "other",
                  "value": "NemSMS"
                }
              ],
              "gender": "female",
              "birthDate": "2020-12-21",
              "address": [
                {
                  "use": "home",
                  "line": [
                    "Scot 2"
                  ]
                }
              ],
              "managingOrganization": {
                "reference": "https://organization.cit-document-transformation-1046.local/fhir/Organization/21734"
              }
            }
          },
          {
            "fullUrl": "https://organization.cit-document-transformation-1046.local/fhir/Organization/58427",
            "resource": {
              "resourceType": "Organization",
              "id": "58427",
              "meta": {
                "versionId": "1",
                "profile": [
                  "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-organization"
                ]
              },
              "extension": [
                {
                  "url": "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-organization-source",
                  "valueCodeableConcept": {
                    "coding": [
                      {
                        "system": "http://ehealth.sundhed.dk/cs/organization-source",
                        "code": "manual"
                      }
                    ]
                  }
                },
                {
                  "url": "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-organization-synchronizationStatus",
                  "valueCodeableConcept": {
                    "coding": [
                      {
                        "system": "http://ehealth.sundhed.dk/cs/organization-synchronization-status",
                        "code": "NotSynchronized"
                      }
                    ]
                  }
                }
              ],
              "identifier": [
                {
                  "use": "official",
                  "system": "urn:oid:2.16.840.1.113883.2.24.1.1",
                  "value": "11111111",
                  "period": {
                    "start": "2020-12-21T13:53:21+00:00"
                  }
                },
                {
                  "system": "urn:oid:1.2.208.176.1.1",
                  "value": "OrgIdentifierValue"
                }
              ],
              "name": "OrgName",
              "telecom": [
                {
                  "system": "phone",
                  "value": "ContactPointValue1",
                  "use": "work"
                }
              ],
              "address": [
                {
                  "use": "work",
                  "line": [
                    "AddressLine1"
                  ],
                  "city": "City1",
                  "postalCode": "PostalCode1",
                  "country": "Country1"
                },
                {
                  "use": "temp",
                  "line": [
                    "AddressLine2"
                  ],
                  "city": "City2",
                  "postalCode": "PostalCode2",
                  "country": "Country2"
                }
              ]
            }
          },
          {
            "fullUrl": "https://organization.cit-document-transformation-1046.local/fhir/Organization/61854",
            "resource": {
              "resourceType": "Organization",
              "id": "61854",
              "meta": {
                "versionId": "1",
                "profile": [
                  "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-organization"
                ]
              },
              "extension": [
                {
                  "url": "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-organization-source",
                  "valueCodeableConcept": {
                    "coding": [
                      {
                        "system": "http://ehealth.sundhed.dk/cs/organization-source",
                        "code": "manual"
                      }
                    ]
                  }
                },
                {
                  "url": "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-organization-synchronizationStatus",
                  "valueCodeableConcept": {
                    "coding": [
                      {
                        "system": "http://ehealth.sundhed.dk/cs/organization-synchronization-status",
                        "code": "NotSynchronized"
                      }
                    ]
                  }
                }
              ],
              "identifier": [
                {
                  "use": "official",
                  "system": "urn:oid:2.16.840.1.113883.2.24.1.1",
                  "value": "11111111",
                  "period": {
                    "start": "2020-12-21T13:53:21+00:00"
                  }
                },
                {
                  "system": "urn:oid:1.2.208.176.1.1",
                  "value": "OrgIdentifierValue"
                }
              ],
              "name": "OrgName",
              "telecom": [
                {
                  "system": "phone",
                  "value": "ContactPointValue1",
                  "use": "work"
                }
              ],
              "address": [
                {
                  "use": "work",
                  "line": [
                    "AddressLine1"
                  ],
                  "city": "City1",
                  "postalCode": "PostalCode1",
                  "country": "Country1"
                },
                {
                  "use": "temp",
                  "line": [
                    "AddressLine2"
                  ],
                  "city": "City2",
                  "postalCode": "PostalCode2",
                  "country": "Country2"
                }
              ]
            }
          }
        ]
      }
    }
  ]
}
```

__Response__
```json
{
  "resourceType": "DocumentReference",
  "meta": {
    "profile": [
      "http://ehealth.sundhed.dk/fhir/StructureDefinition/ehealth-documentreference"
    ]
  },
  "masterIdentifier": {
    "value": "1.2.208.184^IdentifierValue1"
  },
  "status": "current",
  "type": {
    "coding": [
      {
        "system": "http://loinc.org",
        "code": "74465-6",
        "display": "Questionnaire Response Document"
      }
    ]
  },
  "indexed": "2020-12-21T13:53:21.550+00:00",
  "content": [
    {
      "attachment": {
        "contentType": "QRD",
        "data": ""
      },
      "format": {
        "system": "urn:oid:1.2.208.184.100.10",
        "code": "urn:ad:dk:medcom:qrd-v1.2:full",
        "display": "Document adheres to HL7 QRD DK profile ver. 1.0."
      }
    }
  ]
}
```